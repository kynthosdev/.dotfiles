    ...
    listen: {
      events: {
        ...
        'schema.deleted' (data: any) {
          ...
          const classNameB = pascalCase(pluralize.singular(data.payload.singular_name)).substring(4)
          removeSync(`./base-models/${className}.ts`)
          removeSync(`./src/modules/${classNameB}`)
          exec('pnpm generate')
        },
      },
