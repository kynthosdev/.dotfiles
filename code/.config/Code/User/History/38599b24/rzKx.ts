    ...
    listen: {
      events: {
        ...
        'schema.deleted' (data: any) {
          ...
          const baseClassName = pascalCase(pluralize.singular(data.payload.singular_name))
          // const className = pascalCase(pluralize.singular(data.payload.singular_name)).substring(4)
          removeSync(`./pinia-orm-models/base/${baseClassName}.ts`)
