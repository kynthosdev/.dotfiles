...
const piniaOrmModelFiles = fastGlob.sync('./base-models/*.ts')
...
modelNames.forEach((fileName) => {
  createEmptyFile(`./pinia-orm-models/${fileName.substring(4)}.ts`)

  mkdirSync(`./src/modules/${fileName.substring(4)}`, { recursive: true })
